...
# Amazon Reviews #############################
aurl <- "https://www.amazon.in/Apple-iPhone-11-Pro-256GB/product-reviews/B00MITLPX2/ref=cm_cr_arp_d_paging_btm_next_2?showViewpoints=1&pageNumber="
amazon_reviews <- NULL
for (i in 1:30){
...
write.table(amazon_reviews,"AppleIphone11Pro.txt",row.names=FALSE)
getwd()
...
# Term "one","using" and "also" repeats more than or equal to 55 times which doesnt add any value to analysis
x1 <- tm_map(x1, removeWords,c('one','using','also'))
x1 <- tm_map(x1, stripWhitespace)

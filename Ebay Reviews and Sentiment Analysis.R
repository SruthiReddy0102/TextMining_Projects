...
# Ebay Reviews #############################
eurl <- "https://www.ebay.com/urw/Apple-iPhone-X-256GB-Space-Gray-Unlocked-A1901-GSM-/product-reviews/239160993?pgn="
ebay_reviews <- NULL
for (i in 1:15){
...
write.table(ebay_reviews,"Apple-iPhone-X-Ebay.txt",row.names=FALSE)
getwd()
...
x <- tm_map(x, function(x) iconv(x, to='UTF-8', sub='byte'))
...
x1 <- tm_map(x1, removeWords,c('full','yes','like','new','overall'))
x1 <- tm_map(x1, stripWhitespace)
...
# Bar Plot after removing above terms word
